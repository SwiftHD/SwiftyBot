...
function getLatestCommit() {
    window.jQuery.ajax({
            ...
            $.get('https://raw.githubusercontent.com/Apostolique/Agar.io-bot/master/bot.user.js?' + Math.floor((Math.random() * 1000000) + 1), function(data) {
                ...
                var myVersion = parseFloat(SwiftyBotVersion + 0.0000); 
...
console.log("Running Swifty Bot!");
(function(f, g) {
    var splitDistance = 710;
    console.log("Swifty Bot!");
    ...
    f.botList.push(["AposBot " + SwiftyBotVersion, findDestination]);

    var bList = g('#bList');
    g('<option />', {value: (f.botList.length - 1), text: "SwiftyBot"}).appendTo(bList);
